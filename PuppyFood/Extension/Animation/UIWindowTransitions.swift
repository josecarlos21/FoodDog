...
public extension UIWindow {
    ...
    func setRootViewController(_ controller: UIViewController, options: TransitionOptions = TransitionOptions()) {
     ...
     func setRootViewController3D(controllerVC:UIViewController) {
        
        if let snapShow = self.snapshotView(afterScreenUpdates: true) {
            controllerVC.view.addSubview(snapShow)
            UIView.animate(withDuration: 1.0, animations: {
                snapShow.layer.opacity = 0
                snapShow.layer.transform = CATransform3DMakeScale(1.5, 1.5, 1.5)
            }) { (_:Bool) in
                snapShow.removeFromSuperview()
            }
        }
        self.rootViewController = controllerVC
        self.makeKeyAndVisible()
    }
}
